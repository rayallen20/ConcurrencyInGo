...
- 第6行:`fanIn := func(done <-chan interface{}, channels ...<-chan interface{}) <-chan interface{}`.常规的`done`channel,用于控制goroutine的关闭;此处使用一个可变长的`<-chan interface{}`用来做后续汇总数据的操作
- 第7行:使用`sync.WaitGroup`等待参数`channels`中所有的channel都被读取完毕
- 第10行:`multiplex := func(c <-chan interface{})`.函数`multiplex()`将从传入的channel中读取数据,然后将读取到的数据放入channel`multiplexedStream`中
- 第22行:`wg.Add(len(channels))`.此处增加`sync.WaitGroup`的计数器值为传入的channel的数量,其目的在于在确保所有的channel都被读取后再关闭函数`fanIn()`返回的channel
